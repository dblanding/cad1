...
class PositionDialog(QDockWidget):
    ...
    def _build_ui(self):
        ...
        self._status_label = QLabel("Select the part or assembly to\nmove in the tree, then click\n'Start Step' below.")
    ...
    def _on_start(self):
        """Begin a new constraint step -- enter WAITING_PICK1."""
        ...
        if self._constraint_type == ConstraintType.DYNAMIC:
            self._set_status(
                f"Dynamic Move: click a reference point\non '{self._moving_node.label}'\n"
                f"(or any child), then click the\ndestination point."
            )
        else:
            self._set_status(
    ...
    def _update_ui_state(self):
        ...
        # Disable method radios mid-step.
    ...
    def is_in_positioning_mode(self) -> bool:
        ...
        return self._state != PositionState.IDLE
